...
<table border="1">
    <tr>
       <td>Codigo_Hora_Extra</td> 
       <td>Codigo_Jornada</td>
       <td>Numero_HoraExtra</td>
	...
	<?php
$sql="SELECT ex.Cod_HExtra,j.Cod_Jornada,ex.Numero_horaextra,ep.Cod_empleados
,th.Cod_Tipo_HE,ex.fecha from horaextra as ex 
JOIN jornada as j on ex.Cod_Jornada=j.Cod_Jornada
JOIN empleados as ep on ex.Cod_empleados=ep.Cod_empleados
JOIN tipohoraextra as th on ex.Cod_Tipo_HE=th.Cod_Tipo_HE " ;
...
/*if(isset($_POST['consulta'])){
	...
	while($mostrar=mysqli_fetch_array($res)){
		...
		<td>'.$mostrar['Numero_horaextra'].'</td>
		<td>'.$mostrar['Cod_empleados'].'</td>
		<td>'.$mostrar['Cod_Tipo_HE'].'</td>
		<td>'.$mostrar['fecha'].'</td>
